...
{
    public partial class CyprusAddressInfoInput : BaseEntityUserControl<InternshipProvider>
    {
        #region [ Properties ]
        ...
        {
            ddlPrefecture.Items.Add(new ListItem("-- επιλέξτε επαρχία --", ""));

            foreach (var item in CacheManager.Prefectures.GetItems().Where(x => x.CountryID == StudentPracticeConstants.CyprusCountryID).OrderBy(x => x.Name))
            {
