...
Imis.Lib.NoGreekCharacters = function (elem, upperCase) {
    ...
    for (var i = 0; i < StrL; i++) {
        ...
        if ((c >= 'Α' && c <= 'Ω') || (c >= 'α' && c <= 'ω') ||
             ...
             c == 'Ώ' || c == 'ω' || c == 'ώ' ||
             c == ' ') {
            alert(Resources.Imis_Lib_NoGreekCharacters);
            elem.value = StrOut;
            return;
        } else {
            change = 1;
...
Imis.Lib.CheckAfm = function (val, e) {
    ...
    if (e.Value) {
        var afm = e.Value;
        e.IsValid = false;
        if (!afm.match(/^\d{9}$/) || afm.replace('0', '') == '') {
            e.IsValid = false;
            return;
        }
...
Imis.Lib.DisableGreekKeyPress = function (e) {
    ...
    if (!isValid) {
        $.prompt(Resources.Imis_Lib_DisableGreekKeyPress);
    }
    return isValid;
}
...
$(function () {
    function fixTitle(el) {
        var $el = $(el);
        if (!($el.attr('data-hint'))) {
            if ($el.attr('title')) {
                $el.attr('data-hint', $el.attr('title'));
                $el.attr('title', '');
            }
        }
    }
    ...
    $('.hint')
        .live('focus', function () {
            ...
            var hint = control.attr('data-hint');

            $('#dxHintPopupValue').html(hint);
            dxHintPopup.SetPopupElementID(id);
            dxHintPopup.Show();
        })
        .live('blur', function () {
            $('#dxHintPopupValue').html('');
            dxHintPopup.Hide();
        })
        .each(function () {
            fixTitle(this);
